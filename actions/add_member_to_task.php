<?php
    ...
    if ($TeKe->is_logged_in() && $TeKe->has_access(ACCESS_CAN_EDIT)) {
        ...
        if ($task->isAssociatedMember($user->getId())) {
            $TeKe->add_system_message(_("Participant is already associated with this task."), 'error');
            $response->setMessages();
            echo $response->getJSON();
            exit;
        }

        if ($task->addAssociatedMember($user)) {
            $response->setStateSuccess();
            $TeKe->add_system_message(sprintf(_("%s added to task %s."), $user->getFullname(), $task->getTitle()));
            $response->setMessages();
        } else {
            $TeKe->add_system_message(_("Participant could not be associated with task."), 'error');
            $response->setMessages();
        }
