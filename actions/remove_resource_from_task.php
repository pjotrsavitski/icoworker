<?php
    ...
    if ($TeKe->is_logged_in() && $TeKe->has_access(ACCESS_CAN_EDIT)) {
        ...
        if (!$task->isAssociatedResource($resource->getId())) {
            $TeKe->add_system_message(_("Resource is not associated with this task."), 'error');
            $response->setMessages();
            echo $response->getJSON();
            exit;
        }

        if ($task->removeAssociatedResource($resource)) {
            $response->setStateSuccess();
            $TeKe->add_system_message(sprintf(_("%s removed from task %s."), $resource->getTitle(), $task->getTitle()));
            $response->setMessages();
        } else {
            $TeKe->add_system_message(_("Resource could not be removed from task."), 'error');
            $response->setMessages();
        }
